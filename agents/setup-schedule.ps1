# DevBolt Tools - Task Scheduler Setup
...
Write-Host "DevBolt Tools - Setting up scheduled agents..."
Write-Host "Project directory: $projectDir"
Write-Host ""

# Remove old developer tasks if they exist
Unregister-ScheduledTask -TaskName "DevBolt-Developer-AM" -Confirm:$false -ErrorAction SilentlyContinue
Unregister-ScheduledTask -TaskName "DevBolt-Developer-PM" -Confirm:$false -ErrorAction SilentlyContinue
...
Register-ScheduledTask -TaskName "DevBolt-HealthCheck" -Action $healthAction -Trigger $healthTrigger -Settings $healthSettings -Description "DevBolt Tools health check agent" -Force
...
Register-ScheduledTask -TaskName "DevBolt-Developer" -Action $devAction -Trigger $devTrigger -Settings $devSettings -Description "DevBolt Tools developer loop - burns through backlog" -Force
...
Register-ScheduledTask -TaskName "DevBolt-Strategist" -Action $stratAction -Trigger $stratTrigger -Settings $stratSettings -Description "DevBolt Tools strategist agent" -Force
...
Register-ScheduledTask -TaskName "DevBolt-Reporter" -Action $reportAction -Trigger $reportTrigger -Settings $reportSettings -Description "DevBolt Tools reporter agent" -Force
...
Write-Host ""
Write-Host "To view tasks: Get-ScheduledTask | Where-Object {`$_.TaskName -like 'DevBolt*'}"
Write-Host "To remove all: Get-ScheduledTask | Where-Object {`$_.TaskName -like 'DevBolt*'} | Unregister-ScheduledTask"
